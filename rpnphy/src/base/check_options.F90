...
#include <rmnlib_basics.hf>
   ...
   if (LIGHTNING_DIAG) then
      if (STCOND(1:3) /= 'MP_')   then
         call msg(MSG_ERROR,'(check_options) option mismatch: with LIGHTNING_DIAG you have to use STCOND=MP_MY2_OLD, MP_MY2, MP_P3')
         return
      endif
   endif
   ...
   if (stcond == 'MP_P3' .and. (p3_ncat < 1 .or. p3_ncat > 4)) then
      write(str512, '(a,i2,a)') '(check_options) STCOND = MP_P3 option P3_NCAT=',p3_ncat,' (MUST BE BETWEEN 1 AND 4)'
      call msg(MSG_ERROR, str512)
      return
   endif
