     ...
     5                  WT,maxd,igwscheme)
C
C     * FEB 08/12 - A.MARTYNOV. USE ECOCLIMAP DRY AND WET ALBEDO
C     *                         PARAMETERIZATION
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!   Just make sure tht initial values of WT are in the 
!   pre-defined boundary
      if (igwscheme.ne.1) then
        do m=1,im
          do i=il1,il2
            maxcap=25.0*0.2-(maxd-sdepth(i,m))*THPORG(3)
            IF(WT(i,m).GT.maxcap) THEN
              WT(i,m)=maxcap
            ENDIF
          enddo
        enddo
      ...
      DO 200 I=IL1,IL2
          DO 150 J=1,IG
              ...
              ELSEIF(SDEPTH(I,M).LT.(ZBOT(J)-DELZ(J)+0.025)) THEN
                  DELZW(I,M,J)=0.0
                  ISAND(I,M,J)=-3
                  SAND(I,M,J)=-3 ! Huziy: save this to reuse for the following time steps
              ELSE
...
C     ATTENTION! ECOCLIMAP DRY AND WET ALBEDO PARAMETERIZATION!!!
              IF(ICTEMMOD.EQ.0) THEN
C                  ALGDRY(I,M)=MIN(0.14+0.0046*SAND(I,M,1), 0.45) ! FOR GLC2000
                  ALGDRY(I,M)=0.10+(0.05+MAX(0.0030*SAND(I,M,1),0.1))*
     1           ((1-0.9*ORGM(I,M,1)**2))
              ELSE
                  ALGDRY(I,M)=MIN(0.14+0.0027*SAND(I,M,1), 0.41) ! FOR CTEM
              ENDIF
C              ALGWET(I,M)=0.08+0.0006*SAND(I,M,1)
              ALGWET(I,M)=ALGDRY(I,M)/2.0
          ELSE
              ALGWET(I,M)=0.0
              ALGDRY(I,M)=0.0
          ENDIF
