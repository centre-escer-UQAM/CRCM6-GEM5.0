...
!grep -iw intent *.[fF]* | grep -iw PCFC
  ...
  ZQSWD     (1:N)      => bus( x(QSWD   ,1,1) : )  !  input diffuse downward solar radiation flux
...
!print*,'class_main ZCLAY          :',minval(ZCLAY),maxval(ZCLAY),sum(ZCLAY)/(N*IG)
!print*,'class_main ZORGM          :',minval(ZORGM),maxval(ZORGM),sum(ZORGM)/(N*IG)
!print*,'class_main ZCANG          :',minval(ZCANG),maxval(ZCANG),sum(ZCANG)/(N)
!print*,'class_main ZQSWD          :',minval(ZQSWD),maxval(ZQSWD),sum(ZQSWD)/(N)
!print*,'class_main ZSAND          :',minval(ZSAND),maxval(ZSAND),sum(ZSAND)/(N*IG)
