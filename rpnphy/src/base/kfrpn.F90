...
#include "phyinput.cdk"
      ...
      call init2nan(detlq,   detlq2, dmf,    dmf2,  dms,   domgdp)
      ...
      call init2nan(uer,     uer2,   umf,    umf2,  wd,    wspd)
...
!                              are put to "0"
         ...
         if (FLAGCONV(I).le.0) then
            ...
            go to 325 !Active point tendencies persist: no calculations done
...
!                         Find the first level above the LCL
        ...
        do NK = LC,KL
         KLCL=NK
         if(PLCLG(I).ge.PP0(I,NK))GO TO 35
         if (nk > kpbl) then
...
!                               variance) at the LCL
        if (associated(wklclp)) then
           wklcla(i) = wklcl0(i,k) + (wklcl0(i,klcl)-wklcl0(i,k))*dlp
        endif
...
!                               Check to see if the parcel is buoyant
        ...
        case ('tke')
           dzz = z0g(i,klcl)-z0g(i,k)
           dtenv = (thv0(i,klcl)-thv0(i,k))/dzz
           ktlcl = kt0(i,k)+(kt0(i,klcl)-kt0(i,k))*dlp
           dtlcl = min(10.*ktlcl*dtenv/sqrt(tkemixg(i)),3.)
        case ('kain04')
           wkl = 100.*(ww0(i,k)+(ww0(i,klcl)-ww0(i,k))*dlp) - &
...
!                               Calculate the tendencies
        ...
        if (TLCLG(I)+DTLCL.gt.TENV) GO TO 45                                            !Initiation of new cloud
        if (deep_cloudobj .and. cowlcl(i) > WU_MIN) GO TO 45  !Pre-existing cloud triggering
...
!                                 LCL.
      ...
      case ('tke')
         wkl = ww0(i,k)+(ww0(i,klcl)-ww0(i,k))*dlp
!!$         stdw = sqrt(tkemixg(i))
!!$         int_max = 100
