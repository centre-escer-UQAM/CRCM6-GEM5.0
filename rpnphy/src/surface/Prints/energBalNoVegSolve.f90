...
print *,'energBalNoVegSolve AAA QSWNET:',QSWNET
  !
  !>
  !!
...
!print *,'peatlandsMod 000 ISNOW, ISNOALB:',ISNOW,ISNOALB
  if (ISNOW == 0) then ! Use usual snow-free bare soil formulation
    do I = IL1,IL2
      if (FI(I) > 0.) then
        TRTOP(I,1) = 0.
!print *,'peatlandsMod AAA i:',i
...
print *,'energBalNoVegSolve BBB QSWNET:',QSWNET
...
print *,'energBalNoVegSolve CCC QSWNET:',QSWNET
print *,'energBalNoVegSolve TRSNOWG:',TRSNOWG(:,1)
print *,'energBalNoVegSolve FSSB  1:',FSSB(:,1)
print *,'energBalNoVegSolve FSSB  2:',FSSB(:,2)
print *,'energBalNoVegSolve ALSNO 1:',ALSNO(:,1)
print *,'energBalNoVegSolve ALSNO 2:',ALSNO(:,2)
...
print *,'energBalNoVegSolve DDD QSWNET:',QSWNET
      do I = IL1,IL2
        if (FI(I) > 0.) then
          QSWNET(I) = QSWNET(I) - QTRANS(I)
        end if
...
print *,'energBalNoVegSolve EEE QSWNET:',QSWNET
!print *,'energBalNoVegSolve: FSSB(17,:):',FSSB(17,:)
!print *,'energBalNoVegSolve: ALSNO(17,:):',ALSNO(17,:)
!print *,'energBalNoVegSolve: QSWNV(17): ',QSWNV(17)
  !
  do I = IL1,IL2
    if (FI(I) > 0.) then
      TZERO(I) = TSTART(I)
...
!print *,'energBalNoVegSolve: call  mossPht'
if (minval(ipeatland) > 0) &   ! KW
...
6280 format(2X,7F12.4)
  ...
  !     * REMAINING CALCULATIONS.
  !
print *,'energBalNoVegSolve AAA QMELT:',QMELT
  do I = IL1,IL2
    if (FI(I) > 0. .and. ITER(I) == 1) then
      QLWOUT(I) = SBC * TZERO(I) * TZERO(I) * TZERO(I) * TZERO(I)
      if (TZERO(I) < TPOTA(I)) then
      ...
      QMELT(I) = QSWNET(I) + QLWIN(I) - QLWOUT(I) - QSENS(I) - QEVAP(I) - &
                 GZERO(I)
print *,'energBalNoVegSolve i,QMELT,QSWNET,QLWIN,QLWOUT,QSENS,QEVAP,GZERO:',QMELT(I),QSWNET(I),QLWIN(I),QLWOUT(I),QSENS(I),QEVAP(I),GZERO(I)
      RESID(I) = 0.0
      if (QMELT(I) < 0.0) then
        QMELT(I) = QMELT(I) + QEVAP(I)
print *,'energBalNoVegSolve i,QMELT,QEVAP:',QMELT(I),QEVAP(I)
        QEVAP(I) = 0.0
        EVAP(I) = 0.0
      end if
    end if
...
print *,'energBalNoVegSolve ZZZ QMELT:',QMELT
  !
  return
