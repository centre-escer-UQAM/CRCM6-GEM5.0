...
subroutine svs(BUS, BUSSIZ, PTSURF, PTSURFSIZ, DT, KOUNT, TRNCH, N, M, NK)
   ...
   use sfc_options, only: atm_external, atm_tplus, radslope, jdateo, &
        use_photo, nclass, zu, zt, sl_Lmin_soil, VAMIN
   use svs_configs
   implicit none
...
!     
      ...
      integer(8), parameter :: MU_JDATE_HALFDAY = 43200 !#TODO: use value from my_jdate_mod
!
!
!---------------------------------------------------------
!  ROUGNESS
...
!
      ...
      CALL HYDRO_SVS ( DT,      & 
           ...
           bus(x(latflw  ,1,1)), &
           bus(x(runofftot ,1,indx_soil)), bus(x(draintot ,1,indx_soil)), N)
