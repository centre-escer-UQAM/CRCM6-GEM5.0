...
print *,'waterBudgetDriver AAA TRPCCS:',TRPCCS(IL1:IL2)
...
print *,'waterBudgetDriver BBB TRPCCS:',TRPCCS(IL1:IL2)
...
print *,'waterBudgetDriver CCC TRPCCS:',TRPCCS(IL1:IL2)
...
print *,'waterBudgetDriver DDD TRPCCS:',TRPCCS(IL1:IL2)
...
print *,'waterBudgetDriver EEE TRPCCS:',TRPCCS(IL1:IL2)
...
print *,'waterBudgetDriver FFF TRPCCS:',TRPCCS(IL1:IL2)
    call waterFlowNonInfiltrate(1, THLQCS, THICCS, TBRWCS, FDUMMY, TDUMMY, & ! Formerly GRDRAN
                                ...
                                IG, IGP1, IGP2, ILG, IL1, IL2, JL, N)
print *,'waterBudgetDriver FCS TBRWCS:',TBRWCS(IL1:IL2,1)
!print *,'waterBudgetDriver FCS TBRWCS:',TBRWCS
  ...
  !
  if (NLANDGS > 0) then
print *,'waterBudgetDriver AAA TBRWGS(1:ILG,1):',TBRWGS(1:ILG,1)
    call soilWaterPhaseChg(TBARGS, THLQGS, THICGS, HCPGS, TBRWGS, HMFG, HTC, & ! Formerly TWCALC
                           ...
                           IG, ILG, IL1, IL2, JL)
print *,'waterBudgetDriver BBB TBRWGS(1:ILG,1):',TBRWGS(1:ILG,1)
...
print *,'waterBudgetDriver CCC TBRWGS(1:ILG,1):',TBRWGS(1:ILG,1)
    call waterFlowInfiltrate(2, THLQGS, THICGS, TBRWGS, BASFLW, TBASFL, RUNFGS, & ! Formerly GRINFL
                             ...
                             NEND, ISIMP, IGDR, &
                             IG, IGP1, IGP2, ILG, IL1, IL2, JL, N)
print *,'waterBudgetDriver DDD TBRWGS(1:ILG,1):',TBRWGS(1:ILG,1)
    call waterFlowNonInfiltrate(2, THLQGS, THICGS, TBRWGS, FDUMMY, TDUMMY, & ! Formerly GRDRAN
                                ...
                                IZERO, IGRN, IGRD, IGDR, &
                                IG, IGP1, IGP2, ILG, IL1, IL2, JL, N)
print *,'waterBudgetDriver EEE TBRWGS(1:ILG,1):',TBRWGS(1:ILG,1)
  ...
  if (NLANDC > 0) then
    ...
    call waterFlowNonInfiltrate(3, THLQCO, THICCO, TBARWC, FDUMMY, TDUMMY, & ! Formerly GRDRAN
                                ...
                                IZERO, IGRN, IGRD, IGDR, &
                                IG, IGP1, IGP2, ILG, IL1, IL2, JL, N)
print *,'waterBudgetDriver FC  TBARWC:',TBARWC(1:ILG,1)
  ...
  if (NLANDG > 0) then
    ...
    call waterFlowNonInfiltrate(4, THLQGO, THICGO, TBARWG, FDUMMY, TDUMMY, & ! Formerly GRDRAN
                                ...
                                IZERO, IGRN, IGRD, IGDR, &
                                IG, IGP1, IGP2, ILG, IL1, IL2, JL, N)
print *,'waterBudgetDriver FG  TBARWG:',TBARWG(1:ILG,1)
...
if (TCAN(I) < 173.16 .or. TCAN(I) > 373.16) then
  print *,'waterBudgetDriver: FCS,TCANS,CHCAPS,sum:',FCS(I),TCANS(I),CHCAPS(I),FCS(I) * TCANS(I) * CHCAPS(I)
  print *,'waterBudgetDriver: FC ,TCANO,CHCAP ,sum:',FC (I),TCANO(I),CHCAP (I),FC(I) * TCANO(I) * CHCAP(I)
  print *,'waterBudgetDriver: FCS*CHCAPS,FC*CHCAP,sum:',FCS(I)*CHCAPS(I),FC(I)*CHCAP(I),FCS(I) * CHCAPS(I) + FC(I) * CHCAP(I)
endif
    ...
    do I = IL1,IL2
      if (IG == 3 .and. J == IG .and. ISAND(I,1) > - 4) then
        ...
        TBAR(I,J) = (FCS(I) * (TBARCS(I,J) + TFREZ) * (DELZW(I,J) * &
                    ...
                    (DELZ(J) - DELZW(I,J)) * HCPSND))
if (TBAR(I,1) < 173.16 .or. TBAR(I,1) > 373.16) then
!if (i==28 .and. jl==50 .and. j==1) then
  print *,'waterBudgetDriver: jl,i,j,TBAR(I,1):',jl,i,j,TBAR(I,1)
  print *,'waterBudgetDriver: FCS,FGS,FC,FG:',FCS(I),FGS(I),FC (I),FG (I)
  print *,'waterBudgetDriver: TBARCS,TBARGS,TBARC,TBARG:',TBARCS(I,J),TBARGS(I,J),TBARC (I,J),TBARG (I,J)
  print *,'waterBudgetDriver: HCPCS,HCPGS,HCPCO,HCPGO:',HCPCS(I,J),HCPGS(I,J),HCPCO(I,J),HCPGO(I,J)
  print *,'waterBudgetDriver: DELZW,DELZ,HCPSND,TFREZ:',DELZW(I,J),DELZ(J),HCPSND,TFREZ
  print *,'waterBudgetDriver: FCS:',FCS(I) * (TBARCS(I,J) + TFREZ) * (DELZW(I,J) * HCPCS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FCS(I) * (DELZW(I,J) * HCPCS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FCS(I) * (TBARCS(I,J) + TFREZ) * (DELZW(I,J) * HCPCS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)/ &
                                    FCS(I) * (DELZW(I,J) * HCPCS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)
  print *,'waterBudgetDriver: FGS:',FGS(I) * (TBARGS(I,J) + TFREZ) * (DELZW(I,J) * HCPGS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FGS(I) * (DELZW(I,J) * HCPGS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FGS(I) * (TBARGS(I,J) + TFREZ) * (DELZW(I,J) * HCPGS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)/ &
                                    FGS(I) * (DELZW(I,J) * HCPGS(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)
  print *,'waterBudgetDriver: FC :',FC (I) * (TBARC (I,J) + TFREZ) * (DELZW(I,J) * HCPCO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FC (I) * (DELZW(I,J) * HCPCO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FC (I) * (TBARC (I,J) + TFREZ) * (DELZW(I,J) * HCPCO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)/ &
                                    FC (I) * (DELZW(I,J) * HCPCO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)
  print *,'waterBudgetDriver: FG :',FG (I) * (TBARG (I,J) + TFREZ) * (DELZW(I,J) * HCPGO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FG (I) * (DELZW(I,J) * HCPGO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND), &
                                    FG (I) * (TBARG (I,J) + TFREZ) * (DELZW(I,J) * HCPGO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)/ &
                                    FG (I) * (DELZW(I,J) * HCPGO(I,J) + (DELZ(J) - DELZW(I,J)) * HCPSND)
end if
if (TBAR(I,1) < 173.16 .or. TBAR(I,1) > 373.16)  stop
