...
C
      ...
      REAL TRUNOF(ILG)      !Temperature of total runoff from soil 
                            !column [K]
      REAL ZBOTW (ILG,IG),WTnew(ILG), DQDIS(ILG,IG), WTSUB
C
C     * INPUT FIELDS.
C
      ...
      REAL QDIS(ILG),  QIN(ILG), DMLIQ(ILG), DMLIQJ(ILG), xsi(ILG,IG),
     1     ZWT(ILG),rsub_sat,EXCW(ILG),SANI(ILG),
     2     ARE(ILG), SLP(ILG),    LEG(ILG)
C
      INTEGER                LZF   (ILG),      NINF  (ILG), 
     1                       IGRN  (ILG), Isat(ILG), Isats(ILG)
C
...
C
      DO 100 I=IL1,IL2
          ...
          ELSE
              TUSED(I)=0.
              DMLIQ(I)=0.0
              QDIS(I)=0.0   ! for a specific case that we dont meet GW because of LZF=ISAT, to avoide any unwanted value for qin 
              QIN(I)=0.0
              DMLIQJ(I)=0.0
          ENDIF
  100 CONTINUE
...
C
     ...
     4            IZERO,IGRD,IGDR,IG,IGP1,IGP2,ILG,IL1,IL2,JL,N,WT,
     5            ZBOTW,QDIS,QIN,Isat,WTnew,DMLIQ,2,KQIN,Ibed,DMLIQJ,
     6            Isats,xsi,ZWT,EXCW,ARE,LEG,SLP,SANI,igwscheme)    
      !
...
c          WTSUB = 0.0 
          DO J = 1,Isat(I)
c             DQDIS(I,J)=0.0 
              DQDIS(I,J)=xsi(I,J)/(DELZW(I,J)*1000.0) 
          ENDDO
126     CONTINUE
      endif
      ...
      DO 150 I=IL1,IL2
          IF(IGRN(I).GT.0)                                       THEN
               NINF(I)=MIN(NINF(I),IGP1,Isat(I)+1)                                                        
          ENDIF                                                                       
  150 CONTINUE
C
      DO 200 I=IL1,IL2
      DO 200 J=MIN(IGP1, Isat(I)+1),1,-1
          IF(IGRN(I).GT.0)                                          THEN
              ZRMDR(I,J)=DELZX(I,J)
              IF(J.LE.LZF(I))                                 THEN
                if (igwscheme.eq.1 .or. J.NE.(Isat(I)+1)) THEN
                  FDT(I,J)=0.0
                endif
              ENDIF
              IF(J.LT.MIN(IGP1,Isat(I)+1))                    THEN
...
C
     ...
     1                            DELZW(I,J)
                    ...
                    IF (J.NE.MIN(IG,Isat(I))) THEN
                       IF(J.EQ.(MIN(IG,Isat(I))-1)
     1                                         ) THEN
                            THINFL=(WADD(I)+WREM+FDT(I,J)-FDT(I,J+1)+
     1                             DMLIQJ(I))/DELZW(I,J)-DQDIS(I,J)     
                       ELSE
                            THINFL=(WADD(I)+WREM+FDT(I,J)-FDT(I,J+1))/
     1                             DELZW(I,J)-DQDIS(I,J)     
                       ENDIF
                    ELSE
                          THINFL=(WADD(I)+WREM+FDT(I,J)-((QIN(I)*
     1                           TUSED(I)-DMLIQ(I))*KQIN(I))-DMLIQJ(I))/
     2                           DELZW(I,J)-DQDIS(I,J)       
                    ENDIF
                    IF(THINFL.LT.THLMIN(I,J)-0.00001)   THEN                                     
                           FDT(I,J+1)=WADD(I)+WREM+FDT(I,J)-THLMIN(I,J)*
     1                                DELZW(I,J)
                    ENDIF
                   endif
     ...
     1                          DELZW(I,J))
                   ...
                   IF (J.EQ.MIN(IG,Isat(I)).AND.TUSED(I).GT.0) THEN
                     WDRA=-QIN(I)*TUSED(I)*KQIN(I)
     1                                       +FDT(I,J)
                     TDRA=FDT(I,J)*TFDT(I,J)-(QIN(I)*TUSED(I)*KQIN(I))*
     1                    TFDT(I,J+1)
c                     WDRA=FDT(I,J)-QIN(I)*TUSED(I)
c                     TDRA=FDT(I,J)*TFDT(I,J)-(QIN(I)*TUSED(I))*
c     2                  TFDT(I,J+1)
                   ELSEIF (J.EQ.MIN(IG,Isat(I)).AND.TUSED(I).EQ.0) THEN
                     WDRA=0
                     TDRA=0.0
                   ELSEIF (J.NE.MIN(IG,Isat(I))) THEN 
                     WDRA=FDT(I,J)-FDT(I,J+1)                                        
                     TDRA=FDT(I,J)*TFDT(I,J)-FDT(I,J+1)*TFDT(I,J+1)                     
                   ENDIF

                   IF(J.EQ.MIN(IG,Isat(I)))   THEN
                     THLIQX(I,J)=(WADD(I)+WREM+WDRA+DMLIQ(I)-DMLIQJ(I))
     1                           /DELZW(I,J)-DQDIS(I,J)
                     TBARWX(I,J)=(TADD(I)+TREM+TDRA+((DMLIQ(I)-
     1                   DMLIQJ(I))-DQDIS(I,J)*DELZW(I,J))*TBARWX(I,J))/
     2                   (THLIQX(I,J)*DELZW(I,J))
                   ELSEIF (J.EQ.(Isat(I)-1)) THEN
                     THLIQX(I,J)=(WADD(I)+WREM+WDRA+DMLIQJ(I))
     1                           /DELZW(I,J)-DQDIS(I,J)
                     TBARWX(I,J)=(TADD(I)+TREM+TDRA+((DMLIQJ(I))-
     1                            DQDIS(I,J)*DELZW(I,J))*TBARWX(I,J))/
     2                            (THLIQX(I,J)*DELZW(I,J))
                   ELSE
                     THLIQX(I,J)=(WADD(I)+WREM+WDRA)/
     1                            DELZW(I,J)-DQDIS(I,J)
                     TBARWX(I,J)=(TADD(I)+TREM+TDRA+(-DQDIS(I,J)*
     1                 DELZW(I,J))*TBARWX(I,J))/(THLIQX(I,J)*DELZW(I,J))

                   ENDIF
! Arman added this for groundwater
! This part has been added as GW was not called 
! for grinfl independently. In this case we have QIN and QDIS, however, 
! we didnt update soil moisture for the soil layer at the top of 
! GW table for QIN. The modifiication to moisture happen using QIN only 
! when the Isat.GT.IG. QIN unit is (m/s)
                   IF (J.EQ.MIN(IG,Isat(I)) .and. IGRD(I).GT.0) THEN
                     IF(THLIQX(I,J).GT.THLMAX(I,J)) THEN
                       IF (THLIQX(I,J).GT.THICEX(I,J)) THEN
                         THLIQX(I,J)=MIN(THLIQX(I,J),THLMAX(I,J)) 
                       ELSEIF (QIN(I).LE.0.0 .
     1                   AND.KQIN(I).GT.0.AND.TUSED(I).GT.0.) THEN
                         QIN(I)=QIN(I)+MAX(THLIQX(I,J)-THLMAX(I,J),0.0)
     1                       *DELZW(I,J)/TUSED(I)
                         THLIQX(I,J)=MIN(THLIQX(I,J),THLMAX(I,J))
                       ENDIF
                     ENDIF
...
C
      DO 700 I=IL1,IL2
      DO 700 K=1,MIN(IGP1,Isat(I)+1)
          IF(igwscheme.ne.1 .and. Isat(I).LT.IGDR(I))  IGDR(I)=Isat(I)
          IF(IGRN(I).GT.0)                                        THEN
     ...
     1                FDT(I,K))/(RUNOFF(I)+FDT(I,K))
              ...
              ENDIF                              
              IF(igwscheme.ne.1 .and.
     1           (K.EQ.(IGDR(I)+1).and.EXCW(I).GT.0.0))
     2                      RUNOFF(I)=RUNOFF(I)+EXCW(I)
          ENDIF
  700 CONTINUE                                                                
C                                                                                  
      RETURN                                                                      
