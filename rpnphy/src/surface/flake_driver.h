...
USE flakem_configure             ! Switches and parameters that configure FLake
...
! Added following a suggestion from Mike Lazare (KW)
!!!  !$OMP THREADPRIVATE (T_sfc_p, T_sfc_n)
...
HTC_Water: IF(h_ice_n_flk.GE.h_Ice_min_flk) THEN    ! Ice exists
  ...
  ELSE IF(T_bot_p_flk.LT.tpl_T_r) THEN   ! Ice exists and T_bot < T_r, molecular heat transfer 
    h_ML_n_flk = h_ML_p_flk                  ! h_ML remains unchanged 
    C_T_n_flk = C_T_p_flk                    ! C_T (thermocline) remains unchanged 
    IF (abs(C_T_n_flk*(depth_w-h_ML_n_flk)) < 1.e-30_ireals) THEN ! Dugas B., 20110928
      print *,'T_bot_p_flk,tpl_T_r,T_wML_n_flk,T_mnw_n_flk,C_T_n_flk,h_ML_n_flk,depth_w=', &
               T_bot_p_flk,tpl_T_r,T_wML_n_flk,T_mnw_n_flk,C_T_n_flk,h_ML_n_flk,depth_w
...
if (N_T_mean<0.) then
  print *,'flake_driver N_T_mean:',N_T_mean,T_wML_p_flk,T_bot_p_flk
  N_T_mean = max(0., N_T_mean)
endif

!print*,'T_wML_p_flk,T_bot_p_flk:',T_wML_p_flk,T_bot_p_flk
!print*,'flake_buoypar,diff:',flake_buoypar(0.5_ireals*(T_wML_p_flk+T_bot_p_flk)),T_wML_p_flk-T_bot_p_flk

!print*,'h_ML_p_flk,depth_w,h_ML_min_flk:',h_ML_p_flk,depth_w,h_ML_min_flk

  IF(h_ML_p_flk.LE.depth_w-h_ML_min_flk) THEN
!if (h_ML_p_flk.GE.depth_w-0.02) print*,'h_ML_p_flk,depth_w,N_T_mean:',h_ML_p_flk,depth_w,N_T_mean
!print*,'h_ML_p_flk,depth_w:',h_ML_p_flk,depth_w,N_T_mean
    N_T_mean = SQRT(N_T_mean/(depth_w-h_ML_p_flk))  ! Compute N                   
!print*,'N_T_mean:',N_T_mean
  ELSE 
