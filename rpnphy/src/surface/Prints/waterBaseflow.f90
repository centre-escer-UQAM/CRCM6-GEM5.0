...
subroutine waterBaseflow (THLIQX, THICEX, TBARWX, ZPOND, TPOND, & ! Formerly WEND
  ...
  !
print *,'waterBaseflow NNN TBARWX:',TBARWX(IL1:IL2,1)
  do J = IG,1, - 1 ! loop 600
    ...
    !
if (j==1) &
print *,'waterBaseflow TADD:',TADD
    do K = 1,IGP1 ! loop 525
      do I = IL1,IL2
        if (IGRN(I) > 0 .and. K <= NINF(I)) then
          WADD(I) = WADD(I) + THLINF(I,J) * ZMAT(I,K,J)
          TADD(I) = TADD(I) + TMOVE(I,K) * THLINF(I,J) * ZMAT(I,K,J)
if (TADD(I) > 1.e10) then
print *,'waterBaseflow i,j,k,TADD:',i,j,k,TADD(I)
print *,'waterBaseflow TMOVE,THLINF,ZMAT:',TMOVE(I,K),THLINF(I,J),ZMAT(I,K,J)
endif
        end if
      end do
    end do ! loop 525
    ...
    do I = IL1,IL2 ! loop 550
      if (IGRN(I) > 0 .and. DELZW(I,J) > 1.0E-4) then
        ...
        TBARWX(I,J) = (TADD(I) + TREM + TDRA) / (THLIQX(I,J) * &
                      DELZW(I,J))
if (i==15 .and. j==1) &
print *,'waterBaseflow TADD,TREM,TDRA,THLIQX,DELZW:',TADD(I),TREM,TDRA,THLIQX(I,J),DELZW(I,J)
!TADD,TREM,TDRA,THLIQX,DELZW:  2.0831610E+31  0.0000000E+00  0.0000000E+00 0.5248631      0.1000000

      end if
    end do ! loop 550
  end do ! loop 600
print *,'waterBaseflow XXX TBARWX:',TBARWX(IL1:IL2,1)
