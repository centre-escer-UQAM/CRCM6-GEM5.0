...
print *,'snowMelt i,HADD,QMELT,DELT,HCONV:',i,HADD,QMELT(I),DELT,HCONV
        if (HADD <= HCONV) then
...
print *,'snowMelt TRMELT,HADD,HCPW,RMELT:',TRMELT,HADD,HCPW,RMELT
          RMELT = RMELT / DELT
          RMELTS = RMELTS / DELT
          ZSNOW (I) = 0.0
          HCPSNO(I) = 0.0
...
print *,'snowMelt i,TR:',i,TR(i)
        TR   (I) = (R(I) * TR(I) + RMELT * TRMELT) / (R(I) + RMELT)
print *,'snowMelt TR,R,RMELT,TRMELT:',TR(i),R(I),RMELT,TRMELT
        R    (I) = R(I) + RMELT
        QMELT(I) = 0.0
        HTCS(I) = HTCS(I) + FI(I) * HCPSNO(I) * (TSNOW(I) + TFREZ) * &
                  ZSNOW(I) / DELT
