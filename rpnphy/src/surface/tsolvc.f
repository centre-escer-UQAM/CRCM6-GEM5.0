...
cld  Substract from the canopy net shortwave radiation the amount that
c    should return to the atmosphere after being reflected by the surface
             QSWNVC(I)=QSWNVC(I)-QSWINV(I)*TRVISC(I)*TRVISC(I)*ALVISG(I)
              QSWNIC=QSWNIC-QSWINI(I)*TRNIRC(I)*TRNIRC(I)*ALNIRG(I)
cld
...
c$$$          WRITE(6,6380) fi(ibad),rc(ibad),QZERO(IBAD),QAC(IBAD)
c$$$          WRITE(6,6380) (fcancmx(ibad,i),i=1,ictem)
c$$$          WRITE(6,6380) (ailcg(ibad,i),i=1,ictem)
c$$$          WRITE(6,6380) (slai(ibad,i),i=1,ictem)
c$$$          WRITE(6,6380) (rmatctem(ibad,i,1),i=1,ictem)
c$$$          WRITE(6,6380) (rmatctem(ibad,i,2),i=1,ictem)
c$$$          WRITE(6,6380) (rmatctem(ibad,i,3),i=1,ictem)
          CALL XIT('TSOLVC',-1)
      ENDIF
C
