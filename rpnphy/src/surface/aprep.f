...
C     * AUG 16/10 - P Bartlett. Correct the leaf area index calculation in loop 150
...
comment OCT 2009  - Yanjun Jiao Insure that we do not have too small areas.
...
! Sasha: set fc to 0 if pai is 0
          IF(PAI(I,1).LT.1.0E-5) FCAN(I, 1)=0.0
          IF(PAI(I,2).LT.1.0E-5) FCAN(I, 2)=0.0
...
! Sasha: set fcs to 0 if pai is 0
          IF(PAIS(I,1).LT.1.0E-5) FCANS(I, 1)=0.0
          IF(PAIS(I,2).LT.1.0E-5) FCANS(I, 2)=0.0
...
C                                                                                 
          ...
          ENDIF
cdebug<<<
c$$$          if (fg (i).lt.1.e-10) fg (i) = 0.
c$$$          if (fgs(i).lt.1.e-10) fgs(i) = 0.
          if (fg (i).lt.1.e-6) fg (i) = 0.
          if (fgs(i).lt.1.e-6) fgs(i) = 0.
cdebug>>>
          FC (I)=MAX(FC (I),0.0)
          FG (I)=MAX(FG (I),0.0)
          FCS(I)=MAX(FCS(I),0.0)
...
!!! disabled - code gives problems if TCAN is wrong on input (LD)
...
C                                                                                 
      ...
      DO 450 I=IL1,IL2                                                            
        IF (ICTEMMOD.EQ.1) THEN
c$$$          RMAT(I,J,1)=RMATC(I,J,1)
c$$$          RMAT(I,J,2)=RMATC(I,J,2)
c$$$          RMAT(I,J,3)=RMATC(I,J,3)
          DO K=1,IG !(LD) Adapt for runs with more than three layers
...
!      print *,'aprep i,j,ISAND,THPOR:',i,j,ISAND(I,J),THPOR(I,J)
