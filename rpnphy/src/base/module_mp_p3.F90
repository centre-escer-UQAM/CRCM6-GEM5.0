...
!                                                                                          !
! This code was originally written by H. Morrison,  MMM Division, NCAR (Dec 20/2).         !
...
!                        cdistr(i,k),logn0r(i,k))
  ...
  ! note that reflectivity from lookup table is normalized, so we need to multiply by N
     ...
     ! to ice lookup table because a minimum Nr of nsmall will be set otherwise even if qr=0)
       ...
       if (present(cloud_bin)) then
          ! calculate a 'binary' cloud fraction (0 or 1) based on supersaturation
          ! (diagnostic field only -- used in GEM radiation interface)
          cloud_bin(i,k) = 0.
          tmp1           = 0.   !cloud liquid
          tmp2           = 0.   !cloud ice
          if (qc(i,k).ge.qsmall .and. sup(i,k).gt.1.e-6) then
             tmp1 = 1.
          endif
          do iice = 1,nCat
             tmp3 = 0.
             if (qitot(i,k,iice).ge.qsmall .and. diag_effi(i,k,iice).lt.100.e-6) then
                tmp3 = 1.
             endif
             tmp2 = tmp2 + tmp3
          enddo
          cloud_bin(i,k) = tmp1 + tmp2
