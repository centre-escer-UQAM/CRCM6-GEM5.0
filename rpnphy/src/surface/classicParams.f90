...
! Variable also used in GEM and therefore transferred from GEM in set_class_const (KW)
! =========================
  real :: DELT   ! Model timestep
  real :: VAMIN  ! minimum wind speed
  real :: SPHAIR ! = 1.00546E3   !< Specific heat of air (\f$J kg^{-1} K^{-1}\f$)
  real :: GRAV   ! = 9.80616     !< Acceleration due to gravity (\f$m s^{-1}\f$)
  real :: VKC    !< Von Karman Constant (-)
  real :: RGAS   ! = 287.04        !< Gas constant (\f$J kg^{-1} K^{-1}\f$) (GCM name: GAS)
  real :: RGASV  ! = 461.50       !< Gas constant for water vapour (\f$J kg^{-1} K^{-1}\f$) (GCM name: 
  real :: SBC    ! = 5.6698E-8     !< Stefan-Boltzmann constant (\f$W m^{-2} K^{-4} \f$) (GCM name: SIGMA)
  real :: TFREZ  ! = 273.16       !< Freezing point of water (K) (GCM name: CELZRO)
  real :: BETA   ! = 1.0
  real :: FACTN  ! = 1.2
  real :: HMIN   ! = 40.0
  real :: PI     ! = 3.1415926535898 !< pi (-) (GCM name: CPI)
