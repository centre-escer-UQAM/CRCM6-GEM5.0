...
!** S/P CCCMARAD - DRIVER ROUTINE FOR RADIATION
      ...
      subroutine cccmarad (d, dsiz, f, fsiz, v, vsiz , &
                           temp, qq, ps, sig, &
                           tau, kount, icpu , &
                           trnch , n , m , nk , nkp, &
...
!
      ...
      real,dimension(n,nkp)    :: shtj,tfull,s_qrt,v12d
...
!           tfull(i,nkp) = 0.5*(temp(i,nk+1)+f(tsrad+i-1))
!           tfull(i,nkp) = temp(i,nk+1)
!           tfull(i,nkp) = f(tsrad+i-1)
!           tfull(i,nkp) : choose either ground temperature or 2m
...
!
        call ccc1_raddriv (f(fsg),f(fsd0),f(fsf0),f(fsv0),f(fsi0), &
