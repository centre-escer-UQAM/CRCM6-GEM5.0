...
cLD     8   EVPPOT, ACOND,  EVAPB,  GT,     QG,
     8   EVPPOT, ACOND,  EVAPB,  GT,     QG,     TSURF,
     9   ST,     SU,     SV,     SQ,     SRH,
...
cLD     3     QLWAVG(ILG),   GT    (ILG),   QG    (ILG),
     3     QLWAVG(ILG),   GT    (ILG),   QG    (ILG),   TSURF (ILG),
...
C
     ...
     1     DELZ  (IG),    DELZW (ILG,IG),ZBOTW (ILG,IG)
      real bterm (ilg,ig),psisat(ilg,ig),grksat(ilg,ig)
C
...
c    rather than here [CLASS main program]. jm sep 06/12)
c
c    (LD) moved calculations in classt, so that wilting point and field capacity
c         values remain available for the duration of the simulation
c         bterm is calculated in the main subroutine
c         psisat, grksat and thpor are calculated in classb

! Huziy: fixes to avoid division by 0
...
c           psisat(i,j)= (10.0**(-0.0131*sandgat(i,j)+1.88))/100.0
c           grksat(i,j)= (10.0**(0.0153*sandgat(i,j)-0.884))*7.0556e-6
c           thpor(i,j) = (-0.126*sandgat(i,j)+48.9)/100.0
c           bterm(i,j)     = 0.159*claygat(i,j)+2.91

          ! Huziy: Avoid division by 0
           if (ISAND(i, j) .le. -3) then
             cycle
           endif
...
C
      DO 60 J=1,IG
      DO 60 I=IL1,IL2
c        IF(J.LE.Isat(I).OR.J.LE.3) THEN
        IF(igwscheme.eq.1 .or. (J.LE.Isat(I).OR.J.LE.3)) THEN
